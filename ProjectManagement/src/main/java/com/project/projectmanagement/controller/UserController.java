...
//				newUser= request.getSession().getAttribute(attribute).toString(); //Isabel
	...
	public String  saveProfile(@ModelAttribute("persons")Person person ) { 
	...
	//	Login login = new Login();
		...
		int tempLog = loginDetails.getLoginId();
		Optional<Login> login2 = loginRepo.findById(tempLog);
		Login login = login2.get();
		model.addAttribute("persons", person);
		model.addAttribute( "id", tempId );
		...
		return "redirect:/updateMyLoginPage";
	}
	@RequestMapping(value = "/updateMyLoginPage", method = { RequestMethod.GET, RequestMethod.POST })
	public String updateMyLoginPage(Model model, @ModelAttribute("persons")Person person,@ModelAttribute("logins")Login login,  HttpSession session, HttpServletRequest request) {
		...
		while (attributes.hasMoreElements()) {
		    ...
		    if (attribute.equalsIgnoreCase("tempId")) {
		    	int tempId = (int) request.getSession().getAttribute(attribute);
		    	model.addAttribute("persons",personRepo.findById(tempId));
		    	model.addAttribute("logins",loginRepo.findByPersonId(tempId));
		    	break;
		    }
		}
		return "updateLoginProfile";
	}
	@PostMapping("/saveLoginProfile")
	public String  saveLoginProfile(@ModelAttribute("persons")Person person,@ModelAttribute("logins")Login login ) { 
